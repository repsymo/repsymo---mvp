...
import { Investment, Option } from "../../../model/investment/investment";
import {
    InvestmentSolver,
    Stage,
...
export class ImComponent extends Page implements OnInit,
    ...
    onNext() {
        ...
        for (let p = 0;
             p < this.model.numberOfPlans;
             p++) {
            const options: Option[] = [];

            for (let o = 0;
                 o < this.model.numberOfOptions;
                 o++) {
                options.push(
                    {
                        cost: 0,
