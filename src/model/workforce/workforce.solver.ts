...
export interface StageRow {
    ...
    /**
     * x value for that row, demand to chose that makes the cost function
     * minimum.
     */
    x: number;
}
...
export class WorkforceSolver {
    ...
    solve(problemModel: Workforce) {
        ...
        const solveStages = () => {
            for (let stageIndex = timeunits - 1;
                 stageIndex >= 0;
                 stageIndex--) {
    ...
    private getFireEmployeeCost(stageId: number): number {
        ...
        // { const stageNumber = stageId++; // Stage ID starts at 0 return
        // baseCost * stageNumber; }
        return baseCost;
    }
    ...
    private solveStage = (stage: Stage) => {
        ...
        const isHiringNewEmployees = (_x: number, demand: number): boolean => {
            // New employees are hired for this TU iff the number of employees
            // from the previous TU is less than those of this TU
            return _x < demand;
        };
        const isFiringExistingEmployees = (
            _x: number,
            demand: number,
        ): boolean => {
            return demand < _x;
        ...
        const calculateCost = (demand: number, _x: number): number => {
            ...
            // If this stage is hiring new employees then sum up the cost of
            // hiring
        ...
        stage.rows.forEach(row => {
            ...
            // Check each value for each row and so compute the minimum of the
            // options
            for (let b = minimumDemand;
                 b <= this.max;
                 b++) {
                options.push({
                    demand: b,
